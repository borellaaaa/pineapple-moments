...
export default function EditAlbumModal({ album, onClose, onSave }) {
  ...
  const [c1, setC1] = useState(album.cover_color || '#F5C800')
  const [c2, setC2] = useState(album.cover_accent || '#3A8C3F')
      ...
      <div className="modal">
        ...
        <div style={{ display:'flex', gap:20, flexWrap:'wrap' }}>
          <div style={{ textAlign:'center', minWidth:120 }}>
            ...
            <p style={{ fontFamily:'var(--font-title)', fontSize:12, color:'var(--dark)', marginTop:8, maxWidth:130, wordBreak:'break-word' }}>{name}</p>
          </div>
          <form onSubmit={submit} style={{ flex:1, minWidth:220, display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div>
              <label style={{ display:'block', fontWeight:700, fontSize:12, color:'var(--dark)', marginBottom:5 }}>Descrição</label>
              <textarea className="input" value={desc} onChange={e=>setDesc(e.target.value)} rows={2} maxLength={200} />
            </div>
            <div>
              ...
              {showEmojis && (
                <div style={{ display:'grid', gridTemplateColumns:'repeat(8,1fr)', gap:4, background:'#f7faf0', border:'2px solid rgba(27,58,31,0.1)', borderRadius:10, padding:8, marginTop:6, maxHeight:130, overflowY:'auto' }}>
                  {EMOJIS.map(em => (
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:6 }}>
                  {PRESETS.map(([a,b],i) => (
                    <button key={i} type="button"
                      style={{ height:28, borderRadius:8, background:`linear-gradient(135deg,${a},${b})`, border: c1===a&&c2===b?'3px solid var(--dark)':'3px solid transparent', cursor:'pointer' }}
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {[['Cor 1',c1,setC1],['Cor 2',c2,setC2]].map(([lbl,val,set]) => (
                    <div key={lbl} style={{ display:'flex', alignItems:'center', gap:10 }}>
                      <span style={{ fontSize:12, fontWeight:700, width:40 }}>{lbl}</span>
