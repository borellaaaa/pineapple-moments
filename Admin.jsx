...
export default function Admin() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    const [statsRes, usersRes, staffRes, violationsRes, retentionRes, scheduleRes] = await Promise.all([
      ...
      supabase.rpc('admin_get_retention_stats'),
      supabase.rpc('admin_get_deletion_schedule'),
    ])
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 24px 0' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {[['users','👥 Usuários'], ['violations','⚠️ Violações'], ['retention','📅 Retenção'], ['staff','🔑 Equipe']].map(([key, label]) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {retention && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 12 }}>
              {[
                ...
                { label: 'Violações antigas (+2 anos)', value: retention.violations_older_2y, icon: '🗑️', color: '#e53935' },
              ].map(s => (
                <div key={s.label} style={{ background: 'white', borderRadius: 12, padding: '16px 18px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderLeft: `4px solid ${s.color}` }}>
                  <div style={{ fontSize: 22 }}>{s.icon}</div>
          ...
          {retention && (
            <div style={{ background: 'white', borderRadius: 14, padding: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <h3 style={{ fontWeight: 800, fontSize: 14, marginBottom: 12, color: '#1B3A1F' }}>📊 Status das Políticas de Retenção</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13 }}>
                {[
                  { label: 'Próxima exclusão agendada', value: retention.next_deletion ? new Date(retention.next_deletion).toLocaleDateString('pt-BR') : 'Nenhuma pendente', ok: true },
                  { label: 'Log técnico mais antigo', value: retention.oldest_log ? new Date(retention.oldest_log).toLocaleDateString('pt-BR') : '—', ok: true },
                  ...
                  { label: 'Violações a limpar (+2 anos)', value: retention.violations_older_2y > 0 ? `${retention.violations_older_2y} registros` : 'Nenhuma', ok: retention.violations_older_2y === 0 },
                ].map(item => (
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#f9f9f9', borderRadius: 8 }}>
          ...
          {isAdmin && (
            <div style={{ background: 'white', borderRadius: 14, padding: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <h3 style={{ fontWeight: 800, fontSize: 14, marginBottom: 6, color: '#1B3A1F' }}>🧹 Limpeza Manual</h3>
              <p style={{ fontSize: 12, color: '#888', marginBottom: 14 }}>Execute manualmente quando necessário. O Supabase não tem cron gratuito — rode periodicamente.</p>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button onClick={async () => {
                  const { data, error } = await supabase.rpc('process_scheduled_deletions')
                  if (error) { toast('Erro: ' + error.message, 'error'); return }
                  toast(`✅ ${data.deleted} conta(s) deletada(s)`, 'success')
                  loadData()
                }} style={{ padding: '10px 16px', background: '#fdecea', color: '#c62828', border: '2px solid #ffcdd2', borderRadius: 10, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                  ⏳ Processar exclusões agendadas
                </button>
                <button onClick={async () => {
                  const { data, error } = await supabase.rpc('cleanup_technical_logs')
                  if (error) { toast('Erro: ' + error.message, 'error'); return }
                  toast(`✅ ${data.deleted_logs} log(s) removido(s)`, 'success')
                  loadData()
                }} style={{ padding: '10px 16px', background: '#e3f2fd', color: '#1565c0', border: '2px solid #bbdefb', borderRadius: 10, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                  📋 Limpar logs +90 dias
                </button>
                <button onClick={async () => {
                  const { data, error } = await supabase.rpc('cleanup_old_moderation_data')
                  if (error) { toast('Erro: ' + error.message, 'error'); return }
                  toast(`✅ ${data.deleted_violations} violação(ões) removida(s)`, 'success')
                  loadData()
                }} style={{ padding: '10px 16px', background: '#fff8e1', color: '#856404', border: '2px solid #ffe082', borderRadius: 10, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                  ⚠️ Limpar violações +2 anos
                </button>
              </div>
            </div>
          )}

          {/* Lista de exclusões agendadas */}
          <div style={{ background: 'white', borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #f0f0f0', fontWeight: 800, fontSize: 14, color: '#1B3A1F' }}>
              📅 Exclusões Agendadas ({deletionSchedule.filter(d => !d.executed).length} pendentes)
            </div>
            {deletionSchedule.length === 0 ? (
              <div style={{ padding: '30px', textAlign: 'center', color: '#888', fontSize: 13 }}>Nenhum agendamento</div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ background: '#f5f5f5' }}>
                      {['Usuário', 'Email', 'Solicitado em', 'Agendado para', 'Motivo', 'Status'].map(h => (
                  ...
                  <tbody>
                    {deletionSchedule.map((d, i) => (
                      <tr key={d.id} style={{ borderTop: '1px solid #f0f0f0', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                        <td style={{ padding: '10px 14px', fontWeight: 600 }}>@{d.username || '—'}</td>
                        <td style={{ padding: '10px 14px', color: '#555' }}>{d.email}</td>
                        <td style={{ padding: '10px 14px', color: '#888', whiteSpace: 'nowrap' }}>{new Date(d.requested_at).toLocaleDateString('pt-BR')}</td>
                        ...
                        <td style={{ padding: '10px 14px', color: '#555' }}>{d.reason}</td>
                        <td style={{ padding: '10px 14px' }}>
                          {d.executed
                            ? <span style={{ background: '#e8f5e9', color: '#2e7d32', padding: '3px 10px', borderRadius: 99, fontSize: 11, fontWeight: 700 }}>✅ Executado</span>
                            : <span style={{ background: '#fff8e1', color: '#856404', padding: '3px 10px', borderRadius: 99, fontSize: 11, fontWeight: 700 }}>⏳ Pendente</span>
                          }
                        </td>
                      </tr>
                    ))}
